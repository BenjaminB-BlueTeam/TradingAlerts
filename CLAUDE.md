...
netlify/functions/
  ...
  generate-alerts.js    ← cron 7h+18h Paris (5h+16h UTC) — génère alertes LG1/LG2 pour J, J+1, J+2
...
src/
  ...
  lib/
    ...
    components/
      ...
      TeamLgBadges.svelte   ← 2 badges LG1 (but 31-45) + LG2 (but ≥80) par equipe. Prop `context` ('home'/'away'/null) → utilise lg1_home_pct/lg1_away_pct ou lg1_after30_pct (overall). Fetch team_lg1_cache par team_id (derniere saison via order by updated_at desc). seasonId optionnel. Cache module + preload pour batch.
...
| `team_lg1_cache` | Stats LG par equipe/saison. Overall : `lg1_after30_pct` + `lg2_pct` + `matches_count`. Contextuel (split home/away, migration 20260517140000) : `lg1_home_pct, lg1_away_pct, lg2_home_pct, lg2_away_pct, matches_home, matches_away`. PK: (season_id, team_id). Refresh quotidien 4h30 UTC (6h30 Paris) par `compute-team-stats.js` | ON | SELECT | — | ALL |
...
- **Calcul stats équipes LG1/LG2** — `compute-team-stats.js` (cron 4h30 UTC / 6h30 Paris, 30 min apres daily-seed) : pour chaque (season_id, team_id), calcule `lg1_after30_pct` (% matchs avec but en 31-45 min, semantique match-level) + `lg2_pct` (% matchs avec but ≥80 min) depuis `h2h_matches`, upsert dans `team_lg1_cache`. Renomme depuis `compute-team-lg1.js` car le cron calcule maintenant LG1 ET LG2.
- **Badges LG1/LG2 par equipe** (2026-05-17) — composant `TeamLgBadges.svelte` reutilisable, 2 badges colorés (vert ≥55%, orange 40-54, rouge <40, gris si n/a). Affichés inline sur `/matches` (card + expand team-detail), `/alerts/[type]`, `/mes-matchs` (prefetch batch par team_id), `/leagues` (tableau equipes, colonnes triables LG1 31-45 + LG2 ≥80 — passe seasonId explicite). **Sur les cards de matchs/alertes, prop `context='home'` ou `'away'` filtre les % sur les matchs joues dans ce contexte uniquement** (l'equipe a domicile montre ses stats domicile, idem pour l'exterieur). /leagues continue d'afficher l'overall combine. La table `alerts` n'ayant pas de season_id, le composant accepte `teamId` seul et resout la derniere saison connue.
