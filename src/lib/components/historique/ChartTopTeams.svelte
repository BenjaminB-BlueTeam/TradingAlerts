<script>
  ...
  import { aggregateByTeam } from '$lib/utils/historyFilters.js';
  ...
  function draw() {
    if (!canvas) return;
    const rows = aggregateByTeam(alerts, { minMatches, topN });
  ...
  let hasData = $derived(aggregateByTeam(alerts, { minMatches, topN }).length > 0);
</script>

<div class="chart-card">
  <div class="chart-card__header">
    <h3 class="chart-title">Top {topN} équipes</h3>
    <span class="muted">min {minMatches} matchs</span>
  </div>
  <div class="chart-body">
    {#if !hasData}
      <div class="empty">Pas assez de données (min {minMatches} matchs par équipe)</div>
    {/if}
